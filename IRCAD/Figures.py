# -*- coding: utf-8 -*-
"""
Created on Wed May 18 11:31:25 2022
...
#%%
in_path = r"D:\PhD\Article2_AI Semantic Segmentation\Data"
...
# in_df['Batchnorm']=in_df['Batchnorm'].astype('category').cat.codes
# in_df['Recall Weights']=in_df['Recall Weights'].astype('category').cat.codes
# in_df['Recall Betas']=in_df['Recall Betas'].astype('category').cat.codes
# in_df['Augmentation']=in_df['Augmentation'].astype('category').cat.codes
...
# df_rf_heatmap = df_heatmap.drop(df_heatmap[df_heatmap['Loss Fn']==0].index) #Remove "combo loss"
# df_rf_heatmap.drop(labels=['Epoch No.', 'Run No.','Loss Fn','fold', 'train', 'val', 'Combo Loss Weight'], axis=1, inplace=True)
...
# df_cl_heatmap = df_heatmap.drop(df_heatmap[df_heatmap['Loss Fn']==1].index) #Remove "Recall Favoured"
# df_cl_heatmap.drop(labels=['Epoch No.', 'Run No.','Loss Fn','fold', 'train', 'val', 'Recall Weights', 'Recall Betas'], axis=1, inplace=True)
...
#%%Testing grounds
#pair = sns.pairplot(df_validation, corner=True)
#multibox = sns.catplot(x='Combo Loss Weight', y='val_F1_mean', hue='Augmentation', col='Batchnorm', data=df_cl_val, kind="box", showfliers=False)
#multibar = sns.catplot(x='Augmentation', y='val_accuracy', hue='Batchnorm', col='Optimiser', data=df_validation, kind="bar")#, showfliers=False)
#bar = sns.barplot(x='Optimiser', y='val_Precision_2', hue='Batchnorm',data=df_validation)
...
#                   data=df_rf_val, showfliers=False)
box = sns.boxplot(x='Augmentation', y='val_Precision_0', hue='Batchnorm',
                  data=df_rf_val, showfliers=False)
#box.despine(left=True)
# Decorations
...
plt.show()
#%% 
#box = sns.catplot(x='Combo Loss Weight', y='val_accuracy', hue='Augmentation', col='Batchnorm',
#                  data=df_cl_val, kind="box", showfliers=False)
...
#%%
#sns.relplot(data=df_validation, x='Dropout', y='val_loss', hue='Optimiser', size='Loss Fn', style='Batchnorm', col='Augmentation')
#%%
